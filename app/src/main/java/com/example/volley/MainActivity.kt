...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val netWorkUrils = NetworkUtil(this)
        if (netWorkUrils.isNetworkConnected()){
            ...
            JsonObjectRequest(
                ...
                object : Response.ErrorListener {
                    override fun onErrorResponse(error: VolleyError?) {
                        Toast.makeText(this@MainActivity, "${error?.networkResponse?.statusCode}", Toast.LENGTH_SHORT).show()
                    }
                }
            )
